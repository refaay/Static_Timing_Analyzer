...
map<string,node*> ck; //FF mapper to clocks
int inputSlew = 33;
				...
				if (lib_early[libName].lib_pin[pin].isInput) // pin is input Refaay
                 {
                   ...
                   if(lib_early[libName].lib_pin[pin].isClock)
                   {
                     ck[instName] = nn;
                   }
                 }
...
void calculate_slew()
{
	for (int i = 0; i < out_branches.size(); i++)
	{
        ...
        new_branch_slew.net = out_branches[i].net;
...
void wireSlewSetup(vector<out_branch_slew_final> ob)
{
  ...
  for(int i = 0; i < ob.size(); i++)
  {
    ...
    double ss = 0;
    bool iopa = false;
    for(int j = 0; j < inp.size()&&!iopa;j++)
    {
      iopa = ob[i].net == inp[j]->name;
    }
    //if(m[ob[i].net] == m["si"]) ss = inputSlew;
    if(iopa) ss = inputSlew;
    else
    for(int j=0; j<m[ob[i].net]->prev.size(); j++){
    //  get<2>(ending->prev[i])
    //vector<tuple<node*,double,double,double,double>> prev

    cout << "=====" << endl;
    cout << (get<2>(m[ob[i].net]->prev[j])) << endl;
      cout<< "====="<<endl;
      if((get<2>(m[ob[i].net]->prev[j])) > ss) ss = get<2>(m[ob[i].net]->prev[j]);
    }
    cout << "Inst: "<< m[ob[i].net]->name <<" Start: " << start << " " << "End: " << end << " " << "Slew: " << ob[i].slew << " ss " << ss << endl << endl;

    //ob[i].slew = sqrt(pow(ob[i].slew,2)+pow(ss,2));
    ...
    for(int i = 0; i < it->second.size(); i++)
    {
      ...
      for(int y = 0; y < yvector.size(); y++)
        if(lib_early[it->second[i]->lib].lib_timing[it->second[i]->name].count(yvector[y]->name)){
            ...
            cout << " " << it->second[i]->lib << " " << it->second[i]->name <<" "<< yvector[y]->name << " " << yvector[y]->CL << endl;
            ...
            cout << "Library: " << it->second[i]->lib << " Inst: " << it->second[i]->inst << " Input Name: " << it->second[i]->name << " Output Name: " << yvector[y]->name << " Slew of Previuos: " << get<2>(it->second[i]->prev[0]) << " Capacitve Load: " << yvector[y]->CL << endl << endl;
            cout << "Delay in Gate: " << d << " Slew in Gate: " << s << endl << endl;
            tuple<node*,double,double,double,double> t;
            get<0>(t) = yvector[y];
            get<1>(t) = d;
            get<2>(t) = s;
        ...
        if(lib_late[oup[i] -> lib].lib_pin[it->first].isClock)
            {
              ...
              for(int j = 0; j < oup[i]->prev.size();j++)
              {
                ...
                if( ck[oup[i]->inst]->name == it->first)
                {
                  oup[i] -> RAT = period + ck[oup[i]->inst] -> early_AAT - lib_late[oup[i] -> lib].lib_timing[it->first][oup[i] -> name].riseConstraint;
                  oup[i] -> early_RAT = ck[oup[i]->inst] -> AAT + lib_early[oup[i] -> lib].lib_timing[it->first][oup[i] -> name].riseConstraint;
...
int main()
{
  ...
  cout << "Start traversal: \n";
    ...
    for(int u=0; u<all_paths.size(); u++){
      {
        ...
        o << "---------------------Path" << u+1;
        if(u == 0)
        o << "(Most Critical Path)";
         o << "--------------------" << endl;
         o << setw(15)<< " NODE NAME " << setw(10) << " LIBRARY NAME "  << setw(10) << " INSTANCE NAME "  << setw(10) << " LATE SLACK " \
          << setw(10) << " EARLY SLACK " << endl;
      }
      for(int i=0; i<all_paths[u].second.size(); i++)
      if(all_paths[u].second[i] != m["so"] && all_paths[u].second[i] != m["si"])
        o << setw(15) <<all_paths[u].second[i]->name << setw(15) << all_paths[u].second[i]->lib  << setw(15) << all_paths[u].second[i]->inst  << setw(15) << all_paths[u].second[i]->slack \
         << setw(15) << all_paths[u].second[i]->early_slack << endl;
    ...
    o << "---------------------More than  average delay Paths --------------------" << endl;
    for(int u=0; u<all_paths.size(); u++){
      {
        // o << all_paths[u].first << " ";
        //o << "---------------------Path" << u+1;
      ...
      bool with = false;
      for(int i=0; i<all_paths[u].second.size()&&!with; i++){
      if(all_paths[u].second[i] != m["so"] && all_paths[u].second[i] != m["si"])
        if(i+1<all_paths[u].second.size())
        for(int r = 0; r<all_paths[u].second[i]->next.size(); r++)
          if((get<0>(all_paths[u].second[i]->next[r])) == all_paths[u].second[i+1])with = (get<1>((all_paths[u].second[i]->next[r]))) > average;
}
          if(with)
          for(int i=0; i<all_paths[u].second.size(); i++)
          if(all_paths[u].second[i] != m["so"] && all_paths[u].second[i] != m["si"])
